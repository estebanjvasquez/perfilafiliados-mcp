...
IMPORTANTE sobre el ALCANCE real de la Cámara Petrolera: sus empresas afiliadas cubren sectores mucho más amplios de lo que suena "petrolero" a primera oída - financiero/fintech, tecnología, legal, criptoactivos y stablecoins (uso real y documentado en comercialización de crudo y pagos a proveedores), entre otros. NUNCA decidas si un término "pertenece" a la industria usando tu propio conocimiento general de qué es petróleo y gas - ese criterio te va a hacer descartar categorías reales que vos no conocés. Si la frase nombra CUALQUIER tipo de producto, servicio, certificación o actividad de negocio concreta, tratala como una posible búsqueda válida (via "single_compound"/"multi_concept"/"ambiguous" según corresponda) y dejá que la búsqueda real decida si hay o no empresas - buscar y no encontrar ninguna es un resultado normal y válido, mucho mejor que declarar "out_of_scope" por error y nunca buscar.
...
- "out_of_scope": reservalo SOLO para frases que NO describen ningún producto, servicio, certificación o actividad de negocio en absoluto - son temas de charla general sin ninguna lectura posible de búsqueda de empresas (ej. "el clima de mañana", "quién ganó el partido de fútbol", "cuál es la capital de Francia", preguntas personales). NUNCA uses esta categoría para un término que SÍ nombra un producto/servicio/actividad concreta solo porque a vos no te suena "petrolero" - esa clase de error ya pasó en producción (ver nota de ALCANCE arriba) y es peor que buscar y no encontrar nada. Tampoco la uses como excusa para evitar el relleno de "search_phrases": si la frase es un producto/servicio real, sigue siendo "single_compound", simplemente sin inventarle palabras (ver nota de abajo). "search_phrases" va [] y "clarification_question" va null.
